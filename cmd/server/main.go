...
func main() {
	...
	healthHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprintf(w, `{"status":"ok","version":%q}`, config.APP_VERSION)
	}
	r.Get("/healthz", healthHandler)
	r.Get("/think-root/api/healthz", healthHandler)
	...
	r.Group(func(r chi.Router) {
		...
		r.Post("/think-root/api/archive-repository/", routers.ArchiveRepository)
		r.Post("/think-root/api/archive-old-repositories/", routers.ArchiveOldRepositories)
		r.Post("/think-root/api/get-archived-repositories/", routers.GetArchivedRepositories)
		r.Patch("/think-root/api/promote-repository/", routers.PromoteRepository)
