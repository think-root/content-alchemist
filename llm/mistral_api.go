...
func MistralAPI(requestBody map[string]any) (string, error) {
	apiKey := config.MISTRAL_TOKEN
	if apiKey == "" {
		return "", fmt.Errorf("MISTRAL_TOKEN is not set in configuration")
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", fmt.Errorf("mistral API returned non-200 status code: %d, body: %s", resp.StatusCode, string(bodyBytes))
	}
